...
function add_optimality_cuts!(MP, SP_obj, SP_dual, x_prev, inputs, iteration)
    # Input Sets
    P = inputs["Demand scenario probabilities"]
    P_f = inputs["Gas price scenario probabilities"]
    P_k = inputs["Weather scenario probabilities"]
    ...
    @constraint(MP, [s in 1:S, f in 1:F, k in 1:K], MP[:u][s,f,k] >= SP_obj[s,f,k,iteration] + sum(SP_dual[r,s,f,k,iteration]*(MP[:x][r]-x_prev[iteration][r]) for r in 1:R) - MP[:ζ], base_name = "cvar_tail_cuts_"*string(iteration))
    @constraint(MP, [s in 1:S, f in 1:F, k in 1:K], MP[:alpha][s,f,k] >= SP_obj[s,f,k,iteration] + sum(SP_dual[r,s,f,k,iteration]*(MP[:x][r]-x_prev[iteration][r]) for r in 1:R), base_name = "optimality_cut_"*string(iteration))
end
