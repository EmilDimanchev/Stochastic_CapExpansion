...
Capacities = CSV.read(string(inputs_path_plots,sep,"capacity.csv"), DataFrame, header=true)
...
Plots.savefig(Grouped_barPlot_Cap,"Results/Plots_with_zones/grouped_Cap_with_zones_RA_CO2Price.pdf")
...
#Plotting emissions
Emission_per_zone = CSV.read(string(inputs_path_plots,sep,"emissions_per_zone.csv"), DataFrame, header=true)
em_zone = zeros(Z)
for i in 1:Z
...
#Plotting generation
Generation_z1 = CSV.read(string(inputs_path_plots,sep,"generation_z1_CN.csv"), DataFrame, header=true)
Generation_z2 = CSV.read(string(inputs_path_plots,sep,"generation_z2_SK.csv"), DataFrame, header=true)
Generation_z3 = CSV.read(string(inputs_path_plots,sep,"generation_z3_GB.csv"), DataFrame, header=true)
Gen_all = zeros(R,Z)
for i in 1:R
...
#Plotting Revenues
Revenues_all_zones = CSV.read(string(inputs_path_plots,sep,"revenue.csv"), DataFrame, header=true)
