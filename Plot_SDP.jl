...
Plots.savefig(Combined_plots_tech,"Plots/RA_combined_plots_tech.pdf")
...
Plots.savefig(groupbar_techmix,"Plots/grouped_RA_without_CO2_Cap.pdf")
#Plot emissions
...
Plots.savefig(Comb_plot_rev,"Plots/rev_D.pdf")
...
#Plot distribution
dis_rev_1 = [109.045,1/3]
dis_rev_2 = [13.886,1/3]
dis_rev_3 = [8206.68,1/3]
rev_comb =zeros(3,2)
rev_comb[1,:] =dis_rev_1
rev_comb[2,:] =dis_rev_2
rev_comb[3,:] =dis_rev_3


histoPlot = StatsPlots.bar(rev_comb[:,1],rev_comb[:,2])

histoPlot =StatsPlots.plot!(Normal((109+13+8206)/3,1))
...
#Plot emmissions
#display(Plots.plot(collect(1:T), emissions[:,2:3] ,lable ="Coal",title = "Emission per Technology",ylabel = "[Ton CO2]", seriestype =[:bar], palette = cgrad(:greys), fill=0, alpha=0.2, layout = (2,1))) #, label =("Nuclear", "Gas","Wind","Solar","Batteries")
...
#dispaly(Plots.groupedbar(nam, rand(5, 2), group = ctg, xlabel = "Groups", ylabel = "Scores",title = "Scores by group and category", bar_width = 0.67,lw = 0, framestyle = :box))
